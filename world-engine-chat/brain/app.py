...
class EvidenceBlock(BaseModel):
    ...
    schemaInfo: Dict[str, str] = Field(
        default_factory=lambda: {"name": "brain.chat.evidence", "version": "1.0.0"}
    )
    ...
    if cur.fetchone()["c"] == 0:
        seed = [
            (
                "lex_state_v1",
                "state",
                "A configuration of a system at a point in time; in code: a set of values stored in memory.",
                ["engine", "runtime", "math"],
            ),
            (
                "lex_lexicon_v1",
                "lexicon",
                "A structured meaning index: symbols→concepts→procedures, queryable by the brain.",
                ["brain", "knowledge", "contracts"],
            ),
            (
                "lex_toolcall_v1",
                "toolcall",
                "A structured command from the brain that asks a tool runtime (UI/server) to execute an action.",
                ["agents", "runtime"],
            ),
        ]
        cur.executemany(
...
def lexicon_resolve(text: str, limit_per_token: int = 1) -> Dict[str, Any]:
    ...
    for tok in tokens:
        ...
        best = h[0]
        token_map.append(
            {"token": tok, "lexId": best.id, "confidence": best.confidence}
        )
        hits[best.id] = best

    return {"hits": [h.model_dump() for h in hits.values()], "tokenMap": token_map}
...
def detect_toolcalls(text: str) -> List[ToolCall]:
    ...
    if m:
        ...
        system_audio = bool(
            re.search(
                r"\b(system\s+audio|with\s+audio|audio\s+on|include\s+audio)\b", t
            )
        )
        toolcalls.append(
            ToolCall(
                name="record_screen",
                args={"durationMs": duration_ms, "systemAudio": system_audio},
            )
        )

    # record audio 10s
    m2 = re.search(r"\brecord\s+audio\b(.*)", t) or re.search(
        r"\bstart\s+audio\s+recording\b(.*)", t
    )
    if m2:
        duration_ms = parse_duration_ms(t, 5000)
        toolcalls.append(
            ToolCall(name="record_audio", args={"durationMs": duration_ms})
        )
    ...
    if m3:
        ...
        toolcalls.append(
            ToolCall(
                name="lexicon_ingest",
                args={"topic": topic, "meaning": meaning_clean, "tags": tags},
            )
        )
...
def _build_response_text(
    req: ChatRequest, tok_map: List[Dict[str, Any]], toolcalls: List[ToolCall]
) -> str:
    ...
    if tok_map:
        ...
        for tm in tok_map[:6]:
            grounding_lines.append(
                f"- {tm['token']} → {tm['lexId']} ({tm['confidence']:.2f})"
            )
...
def brain_chat(req: ChatRequest) -> ChatResponse:
    ...
    # Build payload
    payload = {"hits": lex_hits, "tokenMap": tok_map}
    ...
    return ChatResponse(
        ...
        evidence=evidence
        if (evidence.grounding or evidence.actions or evidence.citations)
        else None,
    )
...
def brain_tool_result(res: ToolResult) -> ChatResponse:
    ...
    evidence = EvidenceBlock(
        actions=[
            EvidenceActionRow(
                name=_safe_str(res.toolName, 128),
                argsPreview=_safe_str(_args_preview(res.result), 512),
            )
        ]
    )

    return ChatResponse(convoId=res.convoId, text=txt, evidence=evidence)
