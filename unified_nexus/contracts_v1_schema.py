...
def make_v1_event(*, event_type: str, ts_ms: int, trace_id: str, seq: int, payload: Dict[str, JsonValue]) -> V1EventEnvelope:
    ...
    return V1EventEnvelope(v=1, event_type=event_type, ts_ms=ts_ms, trace_id=trace_id, seq=seq, payload=payload, event_id=event_id)


def make_v1_command(*, command_type: str, ts_ms: int, trace_id: str, payload: Dict[str, JsonValue]) -> V1CommandEnvelope:
    validate_jsonable(payload)
    tmp: Dict[str, Any] = {
        "v": 1,
    ...
    return V1CommandEnvelope(v=1, command_type=command_type, ts_ms=ts_ms, trace_id=trace_id, payload=payload, command_id=command_id)


def make_v1_imprint(*, ts_ms: int, trace_id: str, kind: str, data: Dict[str, JsonValue]) -> V1Imprint:
    ...
    return V1Imprint(v=1, imprint_id=imprint_id, ts_ms=ts_ms, trace_id=trace_id, kind=kind, data=data)
