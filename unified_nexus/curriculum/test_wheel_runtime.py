...
def plan() -> dict[str, Any]:
    """Load test plan"""
    return cast(dict[str, Any], WheelRuntime.load_json("schemas/curriculum/wheel.plan.v1.json"))
...
class TestWheelRuntimeBasics:
    ...
    def test_init_validates_plan(self, plan: dict[str, Any], state: dict[str, Any], mock_bus: MagicMock) -> None:
    ...
    def test_init_validates_state(self, plan: dict[str, Any], state: dict[str, Any], mock_bus: MagicMock) -> None:
        """Runtime should validate state on init"""
        runtime = WheelRuntime(plan=plan, state=state, bus=mock_bus)
        assert runtime.state["wheel_id"] == state["wheel_id"]

    def test_next_seq_increments(self, plan: dict[str, Any], state: dict[str, Any], mock_bus: MagicMock) -> None:
        """_next_seq should increment state seq and return new value"""
        runtime = WheelRuntime(plan=plan, state=state, bus=mock_bus)
        assert runtime.state["seq"] == 0

        seq1 = runtime._next_seq()
        assert seq1 == 1
        assert runtime.state["seq"] == 1

        seq2 = runtime._next_seq()
        assert seq2 == 2
...
class TestStatePersistence:
    ...
    def test_save_and_load_state(self, plan: dict[str, Any], state: dict[str, Any], mock_bus: MagicMock) -> None:
    ...
    def test_resumption_preserves_seq(self, plan: dict[str, Any], state: dict[str, Any], mock_bus: MagicMock) -> None:
        ...
        try:
            ...
            seq_after_resume = runtime2._next_seq()
...
class TestEventSequencing:
    ...
    @pytest.mark.asyncio
    async def test_tick_increments_seq(self, plan: dict[str, Any], state: dict[str, Any], mock_bus: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_emitted_events_have_seq(self, plan: dict[str, Any], state: dict[str, Any], mock_bus: MagicMock) -> None:
...
class TestGuardianInvariants:
    ...
    def test_guardian_checks_total_rotations(self, plan: dict[str, Any], state: dict[str, Any], mock_bus: MagicMock) -> None:
    ...
    def test_guardian_checks_stop_index(self, plan: dict[str, Any], state: dict[str, Any], mock_bus: MagicMock) -> None:
        """Runtime should reject if stop_index >= len(stop_order)"""
        state["stop_index"] = len(plan["stop_order"]) + 1
        with pytest.raises(AssertionError):
            WheelRuntime(plan=plan, state=state, bus=mock_bus)

    def test_guardian_checks_wheel_id_match(self, plan: dict[str, Any], state: dict[str, Any], mock_bus: MagicMock) -> None:
...
class TestCommandHandling:
    ...
    @pytest.mark.asyncio
    async def test_on_command_advances_state(self, plan: dict[str, Any], state: dict[str, Any], mock_bus: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_on_command_marks_completed(self, plan: dict[str, Any], state: dict[str, Any], mock_bus: MagicMock) -> None:
...
class TestDeterminismAndRepeat:
    ...
    def test_same_seed_same_picks(self, plan: dict[str, Any], state: dict[str, Any], mock_bus: MagicMock) -> None:
    ...
    def test_anti_repeat_logic(self, plan: dict[str, Any], state: dict[str, Any], mock_bus: MagicMock) -> None:
