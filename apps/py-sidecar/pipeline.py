...
from persist import MergePolicy, open_db, upsert_batch, upsert_lexicon_entries, upsert_rune_rows
...
def main() -> int:
    ...
    ap.add_argument("--language", required=True, help="Language label (TypeScript/Python/...)")
    ap.add_argument("--objective", required=True, help="Objective function for this batch")
    ap.add_argument("--narrative", default="slice_of_life", help="Weekly report narrative mode")
    ap.add_argument("--output-dir", required=True, help="Directory to write artifacts")
    ap.add_argument("--min-confidence", type=float, default=0.80, help="Confidence gate threshold")
    ap.add_argument("--merge-policy", default="2.0", choices=["1.0", "2.0", "3.0"], help="Policy version (MVP supports 2.0 behavior)")
    args = ap.parse_args()
    ...
    write_json(os.path.join(out_dir, "lexicon_entries.json"), transformed["lexicon_entries"])
    write_json(os.path.join(out_dir, "rune_rows.json"), transformed["rune_rows"])

    # 3) Analyst
    plan = run_gates(packet=packet, transform_out=transformed, min_confidence=float(args.min_confidence))
    ...
    if decision["approved_for_release"]:
        ...
        merge_result = upsert_lexicon_entries(conn, transformed["lexicon_entries"], policy)
    ...
    weekly = make_weekly_ops_report(conn=conn, week_starting="2026-02-10", narrative_mode=args.narrative)
