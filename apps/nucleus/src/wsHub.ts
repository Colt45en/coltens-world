...
import { createBusLikeAdapter, createHubPresenceAdapter } from "./health/adapter";
...
const toolExecutorsBySession = new Map<string, ToolExecutor>();
...
type WorldGraphState = MessageMap["world.graph.world_state"]["state"];
...
function createDefaultWorldGraphState(): WorldGraphState {
  return {
    time: 0,
    events_processed: 0,
    entities: [
...
function decodeWorldGraphDescription(description: string): WorldGraphState {
  const lines = description
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter((line) => line.length > 0);

  const entities: WorldGraphState["entities"] = [];
  const systems: WorldGraphState["systems"] = [];
...
export function createHub(wss: WebSocketServer): void {
  ...
  wss.on("connection", (ws: WebSocket, req: IncomingMessage) => {
    ...
    let worldGraphTickTimer: ReturnType<typeof setTimeout> | null = null;
    ...
    const sendWorldGraph = <T extends keyof MessageMap>(
      type: T,
      ...
      send(ws, {
        ...
        nonce: randomId("n"),
        payload,
      } as BusEnvelope<T, MessageMap[T]>);
    };

    const tickWorldGraph = () => {
      if (!worldGraphRunning) return;
    ...
    ws.on("message", async (buf: unknown) => {
      ...
      if (env.type === "system.hello") {
        ...
        if (upgradePath.startsWith("/ws/bus") && requested === "ide") {
          try {
      ...
      if (env.type === "world.graph.set_speed") {
        const payload = env.payload as MessageMap["world.graph.set_speed"];
      ...
      if (env.type === "world.graph.start_simulation") {
        const payload = env.payload as MessageMap["world.graph.start_simulation"];
        if (payload.speed) worldGraphSpeed = payload.speed;
      ...
      if (env.type === "world.graph.generate_world") {
        const payload = env.payload as MessageMap["world.graph.generate_world"];
        try {
      ...
      if (env.type === "uee") {
        ...
        if (!parsedUEE.ok) {
          const errorMsg: unknown = {
            ...
            sessionId,
            auth: { kind: "session" as const, token: client.token },
            nonce: randomId("n"),
            payload: { error: parsedUEE.message },
          };
          send(ws, errorMsg as any);
          return;
        }
      ...
      if (env.type === "brain.chat") {
        await chatHandler.handleBrainChat(env as any, ws);
        return;
      }
      ...
      if (env.type === "tool.effect.v1") {
        ...
        if (executor) {
          executor.handleToolEffectFromIde(env as any);
        }
        return;
      }
      ...
      if (env.type === "ide.fs.read.request") {
        const p = env.payload as MessageMap["ide.fs.read.request"];
        let ok = true;
        ...
        try {
          ...
          if (stats.size > p.maxBytes) {
            throw new Error(
              `File size (${stats.size} bytes) exceeds limit (${p.maxBytes} bytes)`
            );
          }
  ...
  startHealthPoller({
    ...
    hubPresence,
    sessionId,
    sidecarBaseUrl: process.env.SIDECAR_URL ?? "http://127.0.0.1:8011",
    intervalMs: 1000,
