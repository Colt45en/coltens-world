...
import type { Capability, Role } from "@world-engine/protocol";
...
export class SessionStore {
  ...
  private nonces = new Map<string, NonceEntry[]>(); // sessionId → nonces
  private rateLimits = new Map<string, RateLimitData>(); // sessionId → rate limit
   ...
   */
  checkNonce(sessionId: string, nonce: string): boolean {
    const now = Date.now();
