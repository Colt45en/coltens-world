...
const BRAIN_ENDPOINT = process.env.BRAIN_ENDPOINT || "http://localhost:8001";
 ...
 */
export async function handleChatRequestStreaming(ws, env, sessionId, token, HUB_INSTANCE_ID, toolExecutorsBySession) {
    const payload = env.payload;
    const traceId = env.id ?? randomId("tr");
    const messageId = payload.messageId ?? randomId("m");
    console.log(`[chat] stream=${true} traceId=${traceId} convoId=${payload.convoId || "default"} prompt="${payload.prompt?.slice(0, 40)}..."`);
    const controller = new AbortController();
    const toolCalls = []; // Collect tool calls during streaming
    // Create tool executor (sends to IDE and sidecar)
    const toolExecutor = new ToolExecutor((e) => send(ws, e), // sendToIde (broadcast)
