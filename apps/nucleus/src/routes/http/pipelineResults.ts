...
export function handlePipelineResultsIndex(
  _req: IncomingMessage,
  res: ServerResponse,
  opts?: { rootDir?: string }
): boolean {
  if (_req.url !== "/api/pipeline/results/index") return false;
...
export function handlePipelineResultsFile(
  req: IncomingMessage,
  res: ServerResponse,
  opts?: { rootDir?: string }
): boolean {
  const url = new URL(req.url ?? "", `http://${req.headers.host}`);
  if (!url.pathname.startsWith("/api/pipeline/results/file/")) return false;
 ...
 */
export function registerPipelineResultsRoutes(
  ...
  if (orig) {
    ...
    server.on("request", (req: IncomingMessage, res: ServerResponse) => {
      if (
        handlePipelineResultsIndex(req, res, opts) ||
