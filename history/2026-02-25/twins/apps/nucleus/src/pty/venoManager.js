...
export class VenoManager {
    ...
    async openVeno(sessionId, opts) {
        ...
        const sess = { ptyId, sessionId, pty: p };
        ...
        p.onData((data) => opts.onData(ptyId, data));
        // Launch veno
    ...
    ensureVenoInstalled() {
        ...
        if (pipRun.status !== 0) {
            throw new Error("Failed to install veno requirements. Ensure Python 3 + pip are installed and available.");
        }
