 ...
 */
import { getByPath, Sandbox, tools } from "./sandbox-tools.js";
console.log("╔════════════════════════════════════════════╗");
...
console.log("Agents:");
const agents = currentState.orchestration?.agents ?? [];
for (const agent of agents) {
    console.log(`  ${agent.name} (${agent.role})`);
}
console.log("Epochs:");
for (const epoch of currentState.epochs ?? []) {
...
for (const path of demoPaths) {
    try {
        const val = getByPath(currentState, path);
        console.log(`  ${path}`);
