...
export const sigmoid = (x) => 1 / (1 + Math.exp(-Math.min(x, 100))); // Clamp to prevent overflow
export const AGENT_BRAIN_INPUT_SIZE = 20;
export const AGENT_BRAIN_OUTPUT_SIZE = 9;
export const DEFAULT_AGENT_BRAIN_HIDDEN_SIZES = [16, 12];
export function createDefaultAgentNetwork(hiddenSizes = [...DEFAULT_AGENT_BRAIN_HIDDEN_SIZES]) {
...
export class NeuralNetwork {
    ...
    hiddenSizes;
    outputSize;
    layers; // Weights for each layer
    biases; // Biases for each layer
    fitness = 0;
    activationHistory = []; // For debugging/visualization
    constructor(inputSize, hiddenSizes, outputSize) {
        ...
        for (let i = 0; i < allSizes.length - 1; i++) {
            ...
            // Random weights [-1, 1]
            this.layers.push(Array.from({ length: prevSize }, () => Array.from({ length: currSize }, () => Math.random() * 2 - 1)));
            // Random biases [-1, 1]
