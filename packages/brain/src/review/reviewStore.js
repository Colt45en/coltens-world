...
import { KnowledgeArtifactSchema, LexiconEntrySchema, ReviewDecisionSchema, ReviewQueueItemSchema, } from "./reviewTypes";
...
export async function loadQueueItemById(opts, id) {
...
export async function promoteReviewItem(params) {
    ...
    if (!item)
        throw new Error(`Item at index ${idx} is undefined`);
    ...
    if (decision === "approve") {
        ...
        if (payload?.artifact) {
            const artifact = KnowledgeArtifactSchema.parse(payload.artifact);
            await appendNdjson(opts.memoryFile, { ...artifact, status: "canonical" });
            wroteMemory = true;
        }
        if (payload?.lexiconEntry) {
            const entry = LexiconEntrySchema.parse(payload.lexiconEntry);
    ...
    const decisionRecord = ReviewDecisionSchema.parse({
        schemaVersion: "1.0.0",
        decisionId,
        itemId: id,
        decidedAt,
...
export async function rebuildLexiconIndex(lexiconEntriesDir) {
