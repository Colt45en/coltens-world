/**
 * Punctuation Governor - enforces consistent spacing & terminal punctuation
 * Ensures all text output follows deterministic rules
 */
...
/**
 * Insert space after punctuation where needed (except before closing brackets/more punct)
 */
...
/**
 * Govern multiple paragraphs (separated by blank lines)
 * Keeps structure intact while normalizing each block independently
 */
export function governMultiParagraph(text: string): string {
  ...
  // Each paragraph governed independently so punctuation doesn't "eat" structure
