...
export type CollisionEvent = {
    entityId: string;
    otherId?: string; // undefined for world collision
    normal: { x: number; y: number }; // push-out direction
    penetration: number; // depth of overlap
};

/**
 * Detect circle-circle collision
 */
export function detectCircleCircle(a: CircleCollider, b: CircleCollider): CollisionEvent | null {
    ...
    if (dist < minDist) {
        ...
        return {
            entityId: a.pos as any, // Placeholder, will be set by caller
            otherId: b.pos as any,
            normal: { x: nx, y: ny },
            penetration
        };
    }
 ...
 */
export function detectCircleBox(circle: CircleCollider, box: BoxCollider): CollisionEvent | null {
    ...
    if (dist < circle.radius) {
        ...
        return {
            entityId: circle.pos as any, // Placeholder
            normal: { x: nx, y: ny },
            penetration
        };
    }
...
export function predictMove(
    ...
    otherEntities?: Map<string, CircleCollider>
): {
    ...
    const testCircle: CircleCollider = {
        type: 'circle',
        radius,
        pos: nextPos
    };
 ...
 */
export function slideAlongCollision(
    pos: { x: number; y: number },
    velocity: { x: number; y: number },
    collision: CollisionEvent
): { pos: { x: number; y: number }; velocity: { x: number; y: number } } {
