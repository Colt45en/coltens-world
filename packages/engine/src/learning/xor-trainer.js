...
function forward(state, x) {
    ...
    const z2 = [h[0] * W2[0][0] + h[1] * W2[1][0] + b2[0]];
    // Output activation: yhat = sigmoid(z2)
    const yhat = [sigmoid(z2[0])];
    return { z1, h, z2, yhat };
}
// ============================================================================
// EVIDENCE EMISSION
 ...
 */
function computeGeometryStats(hiddenBatch) {
 ...
 */
function classifyDetector(h, inputs) {
    const [x1, x2] = inputs;
    const [h0, h1] = h;
 ...
 */
function computeSeparability(hiddenBatch, yBatch) {
    if (hiddenBatch.length === 0 || yBatch.length === 0)
        return 0;
    ...
    for (let i = 0; i < minLen; i++) {
        ...
        const y = yBatch[i];
        if (y === 0)
            w0 += Math.hypot(h[0] - m0x, h[1] - m0y);
        else
            w1 += Math.hypot(h[0] - m1x, h[1] - m1y);
    }
...
function classifyUpdatePhase(epoch, learningRate) {
    if (epoch < learningRate * 100)
        return "EarlyTraining";
    if (epoch < 0.8 * 20000)
        return "Convergence";
    return "Finetuning";
}
function getChangeReason(detectorType) {
    if (detectorType === "OR")
        return "ORDetectorFormation";
    if (detectorType === "AND")
        return "ANDDetectorFormation";
    if (detectorType === "NOT_AND")
        return "NOT_ANDDetectorFormation";
    return "SelectivityRefinement";
}
function getSeparabilityState(separability) {
    if (separability < 0.4)
        return "Fuzzy";
    if (separability < 0.7)
        return "PartialyySeparable";
    return "LinearySeparable";
}
/**
 * Emit representation evidence packet for a hidden unit
 */
function emitEvidencePacket(input) {
    const { state, unitId, inputContext, hiddenValue, delta, oldWeights, newWeights, lossBeforeUpdate, lossAfterUpdate, hiddenBatch, yBatch, batchIndex, } = input;
    ...
    const updatedDirectionCorrect = lossAfterUpdate <= lossBeforeUpdate;
    const packet = {
        version: "1.0",
        ...
        hiddenActivation: {
            unitId,
            value: hiddenValue,
            inputContext: [...inputContext], // Convert readonly to mutable
        },
        blameSignal: {
            delta,
            source: "OutputLayer",
...
function createTrainingEvidencePacket(input) {
    ...
    const h_readonly = [h[0], h[1]];
    const delta1_readonly = [delta1[0], delta1[1]];
...
function runTrainingEpoch(state, X, Y, emitEvidence) {
    const dW1 = [[0, 0], [0, 0]];
    const db1 = [0, 0];
    const dW2 = [[0], [0]];
    const db2 = [0];
    ...
    for (let sampleIdx = 0; sampleIdx < X.length; sampleIdx++) {
        ...
        db1[1] += delta1[1];
        if (emitEvidence) {
            evidencePackets.push(createTrainingEvidencePacket({
                state,
                sampleIdx,
                x,
                h,
...
function logEpochProgress(state, X, epoch) {
    const predictions = [];
    for (const xSample of X) {
        const hVal = forward(state, xSample).yhat[0];
        predictions.push(hVal);
    }
    console.log(`epoch=${epoch} loss=${state.loss.toFixed(6)} preds=[${predictions.map((p) => p.toFixed(3)).join(", ")}]`);
}
function logFinalEvaluation(state, X, Y) {
    console.log("\n=== Final XOR Network ===");
    for (let i = 0; i < X.length; i++) {
        ...
        const h = fwd.h;
        const yVal = Y[i] ?? 0;
        console.log(`x=${JSON.stringify(xSample)} y=${yVal} yhat=${fwd.yhat[0].toFixed(3)} hidden=[${h.map((v) => v.toFixed(3)).join(", ")}]`);
    }
}
function formatBoundaryEquation(wX, wY, bias) {
...
export function analyzeXORBoundaries(state) {
    ...
    const points = xorPoints.map(([input, target]) => {
        const f = forward(state, input);
        const h0 = f.h[0] >= 0.5 ? 1 : 0;
        const h1b = f.h[1] >= 0.5 ? 1 : 0;
        return {
    ...
    return {
        lines,
        points,
        note: "Hidden units define two soft linear boundaries. Their binary activation code (>=0.5) re-encodes XOR points into a space the output neuron can separate.",
    };
}
const DEFAULT_XOR_TRAINER_CONFIG = {
    epochs: 20000,
    learningRate: 0.5,
    verbose: true,
    emitEvidence: true,
 ...
 */
export function trainXORNetwork(config) {
