 ...
 */
export function coerceJsonValue(x: unknown): JsonValue {
    if (x === null) return null;
    if (x === undefined) return null;
    if (typeof x === "string") return x;
    if (typeof x === "number") {
        if (!Number.isFinite(x)) return null; // NaN, Infinity → null
        return x;
    }
    ...
    if (Array.isArray(x)) {
        ...
        const walk = (val: unknown): JsonValue => {
    ...
    if (typeof x === "object") {
        const seen = new WeakSet();
        if (seen.has(x)) return null; // circular
        seen.add(x);
 ...
 */
export function requireJsonObject(x: unknown): JsonObject {
 ...
 */
export function requireJsonValue(x: unknown): JsonValue {
